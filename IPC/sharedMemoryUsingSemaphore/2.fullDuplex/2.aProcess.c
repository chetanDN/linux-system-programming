...
main()
{
	...
	semid=semget(4,5,IPC_CREAT|0644);	//creating 5 sub semaphores( but using last 4 only)
	if(semid<0)
	{
		perror("semget");
		return;
	...
	if(fork())
	{
		...
		{
			semop(semid,&v,1);
		 	semctl(semid,2,SETVAL,1);	//set lock to process 2 child
			scanf("%s",shmp1);
			semctl(semid,1,SETVAL,1);	//lock self
			semctl(semid,2,SETVAL,0);	//open lock to process 2 chld
		}
	}
	else
	{
		...
		{
			semop(semid,&v,1);
			semctl(semid,3,SETVAL,1);	//set lock to process 2 parent
			printf("data p1 rx=%s\n",shmp2);
			semctl(semid,4,SETVAL,1);	//self lock
			semctl(semid,3,SETVAL,0);	//open lock for process 2 parent
		}
	}
}
